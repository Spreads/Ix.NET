...
namespace Spreads.Ix {
    public static partial class AsyncEnumerable {
        ...
        class AnonymousAsyncEnumerator<T> : IAsyncEnumerator<T> {
            ...
            private Task<bool> _asyncMoveTask;
            ...
            public Task<bool> MoveNext(CancellationToken cancellationToken) {
                if (_disposed) { return TaskExt.False; }

                if (_asyncMoveTask == null) return _moveNext(cancellationToken);

                // NB We could use CompareExchange here. However, enumerators are single-threaded and "very thread-unsafe". A multi-threaded usage of an enumerator is an application error.
                var t = _asyncMoveTask;
                _asyncMoveTask = null;
                return t;
            }
            ...
            public bool MoveNext() {
                var moveTask = MoveNext(CancellationToken.None);
                if (moveTask.IsCompleted) {
                    return moveTask.Result;
                }

                // NB sync MN returns immediately. If the task was not completed (i.e. was not a TCS.Task or Task.FromResult) then we store it for next async move
                _asyncMoveTask = moveTask;
                return false;
            }

            public void Reset() {
                throw new NotSupportedException("Reset is not supported in Ix extensions");
            }
        }
